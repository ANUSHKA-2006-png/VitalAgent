...
OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "stress" / "temp"

TEMP_FS = 4
LABEL_FS = 700
WINDOW_SECONDS = 8

TEMP_WINDOW = TEMP_FS * WINDOW_SECONDS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess WESAD wrist TEMP into 8-second stress windows."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip compatibility checks against existing BVP and EDA metadata.",
    )
...
def iter_subject_files(raw_root: Path) -> list[Path]:
    # Match the original BVP/EDA preprocessing order, which used plain Path sorting.
...
def metadata_matches_bvp(temp_metadata: pd.DataFrame, bvp_metadata: pd.DataFrame) -> bool:
    exact_columns = ["subject", "window_id", "wesad_label", "stress_label"]
    time_columns = ["start_time_seconds", "end_time_seconds"]

    if len(temp_metadata) != len(bvp_metadata):
        return False
    for column in exact_columns:
        if not temp_metadata[column].reset_index(drop=True).equals(
            bvp_metadata[column].reset_index(drop=True)
        ):
            return False
    for column in time_columns:
        if not np.allclose(
            temp_metadata[column].to_numpy(dtype=float),
...
def metadata_matches_eda(temp_metadata: pd.DataFrame, eda_metadata: pd.DataFrame) -> bool:
    exact_columns = [
        "subject",
        "window_id",
        "start_sample",
    ...
    if len(temp_metadata) != len(eda_metadata):
        return False
    for column in exact_columns:
        if not temp_metadata[column].reset_index(drop=True).equals(
            eda_metadata[column].reset_index(drop=True)
        ):
            return False
    for column in time_columns:
        if not np.allclose(
...
def check_reference_alignment(metadata: pd.DataFrame) -> None:
    ...
    if not EDA_METADATA_PATH.exists():
        raise FileNotFoundError(f"Existing EDA metadata not found: {EDA_METADATA_PATH}")
    ...
    if not metadata_matches_bvp(metadata, bvp_metadata):
        raise ValueError("TEMP metadata does not align with existing BVP metadata.")
    if not metadata_matches_eda(metadata, eda_metadata):
        raise ValueError("TEMP metadata does not align with existing EDA metadata.")
...
def build_splits(
    ...
    if any(overlaps.values()):
        raise ValueError(f"Subject overlap detected in TEMP splits: {overlaps}")
...
def main() -> None:
    ...
    print("=" * 70)
    print("VITALAGENT - WESAD TEMP PREPROCESSING")
    print("=" * 70)
    print(f"Raw root: {raw_root}")
    print(f"Output: {output_dir}")
    print(f"TEMP sampling rate: {TEMP_FS} Hz")
    print(f"Window: {WINDOW_SECONDS} seconds / {TEMP_WINDOW} samples")
    ...
    for pkl_path in subject_files:
        ...
        inferred_temp_fs = infer_sampling_rate(len(temp), len(labels))
        if not np.isclose(inferred_temp_fs, TEMP_FS, atol=1e-6):
            raise ValueError(
                f"{subject}: inferred TEMP sampling rate {inferred_temp_fs:.6f} "
                f"does not match expected {TEMP_FS} Hz."
            )

        num_windows = min(len(temp) // TEMP_WINDOW, len(labels) // LABEL_WINDOW)
        accepted_windows = 0

        print(f"Available wrist signals: {sorted(wrist.keys())}")
        print(f"TEMP samples: {len(temp)}")
        print(f"Label samples: {len(labels)}")
        print(f"Inferred TEMP sampling rate: {inferred_temp_fs:.2f} Hz")
        ...
        raw_summary_rows.append(
            {
                ...
                "temp_sampling_rate_hz": float(inferred_temp_fs),
        ...
        for window_id in range(num_windows):
            temp_start = window_id * TEMP_WINDOW
            ...
            X_all.append(temp[temp_start:temp_end])
            y_all.append(stress_label)
            metadata_rows.append(
                {
                    "subject": subject,
                    "window_id": int(window_id),
                    "start_sample": int(temp_start),
                    "end_sample": int(temp_end),
                    "start_time_seconds": float(temp_start / TEMP_FS),
                    "end_time_seconds": float(temp_end / TEMP_FS),
                    "label_start_sample": int(label_start),
                    "label_end_sample": int(label_end),
                    "temp_sampling_rate_hz": TEMP_FS,
        ...
        print(f"Complete 8-second TEMP windows: {num_windows}")
        print(f"Accepted windows: {accepted_windows}")

        del data, wrist, temp, bvp, eda, acc, labels
        gc.collect()
    ...
    if X.ndim != 2 or X.shape[1] != TEMP_WINDOW:
        raise ValueError(f"Expected TEMP X shape (n, {TEMP_WINDOW}), got {X.shape}")
    if len(X) != len(y) or len(y) != len(metadata):
        raise ValueError("TEMP X, y, and metadata row counts do not match.")
    if not np.isfinite(X).all():
        raise ValueError("TEMP X contains NaN or Inf values.")

    if not args.skip_reference_alignment_check:
        check_reference_alignment(metadata)
        print("\nTEMP metadata alignment with existing BVP and EDA metadata: OK")
    ...
    print("\nWESAD TEMP preprocessing completed successfully.")
