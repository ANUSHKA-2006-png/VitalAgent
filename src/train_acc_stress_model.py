...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an ACC-only WESAD stress classifier.")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    parser.add_argument("--artifact-path", type=Path, default=MODALITY_MODEL_ROOT / "acc_model.joblib")
    parser.add_argument("--metrics-path", type=Path, default=MODALITY_METRICS_DIR / "acc_metrics.json")
...
def main() -> None:
    ...
    print("=" * 70)
    print("VITALAGENT - ACC STRESS MODEL TRAINING")
    print("=" * 70)

    X_train, y_train, _ = load_split("acc", "train")
    X_val, y_val, _ = load_split("acc", "val")
    X_test, y_test, _ = load_split("acc", "test")
    ...
    X_train_features = build_feature_matrix(X_train, "acc")
    X_val_features = build_feature_matrix(X_val, "acc")
    X_test_features = build_feature_matrix(X_test, "acc")
