...
from process_wesad_temp import (
    EDA_METADATA_PATH,
    NON_STRESS_LABELS,
    OUTPUT_DIR,
    PROJECT_ROOT,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify processed WESAD wrist TEMP windows.")
    parser.add_argument("--temp-dir", type=Path, default=OUTPUT_DIR)
...
def validate_dataset(
    ...
    if X.ndim != 2 or X.shape[1] != TEMP_WINDOW:
        raise ValueError(f"{name}: expected X shape (n, {TEMP_WINDOW}), got {X.shape}")
    if y.ndim != 1:
    ...
    if not np.all(sample_lengths.to_numpy(dtype=int) == TEMP_WINDOW):
        raise ValueError(f"{name}: not all TEMP windows contain {TEMP_WINDOW} samples.")

    if "temp_sampling_rate_hz" in metadata.columns:
        if not np.all(metadata["temp_sampling_rate_hz"].to_numpy(dtype=float) == TEMP_FS):
            raise ValueError(f"{name}: unexpected TEMP sampling-rate metadata.")


def verify_reference_alignment(temp_dir: Path) -> None:
    _, _, temp_metadata = load_dataset(temp_dir)
    bvp_metadata = pd.read_csv(BVP_PROCESSED_DIR / "metadata.csv")
    eda_metadata = pd.read_csv(EDA_METADATA_PATH)

    full_bvp_match = metadata_matches_bvp(temp_metadata, bvp_metadata)
    full_eda_match = metadata_matches_eda(temp_metadata, eda_metadata)

    print("\nReference alignment")
    print("-------------------")
    print(f"Full TEMP metadata aligns with existing BVP metadata: {full_bvp_match}")
    print(f"Full TEMP metadata aligns with existing EDA metadata: {full_eda_match}")

    if not full_bvp_match:
        raise ValueError("Full TEMP metadata does not align with existing BVP metadata.")
    if not full_eda_match:
        raise ValueError("Full TEMP metadata does not align with existing EDA metadata.")

    split_subjects: dict[str, set[str]] = {}
    for split, expected_subjects_list in SPLIT_SUBJECTS.items():
        _, _, temp_split_metadata = load_dataset(temp_dir, split)
        ...
        split_subjects[split] = temp_subjects

        subjects_match = temp_subjects == expected_subjects == bvp_subjects == eda_subjects
        bvp_match = metadata_matches_bvp(temp_split_metadata, bvp_split_metadata)
        eda_match = metadata_matches_eda(temp_split_metadata, eda_split_metadata)
        ...
        if not subjects_match:
            raise ValueError(f"{split}: TEMP subjects do not match expected/BVP/EDA subjects.")
        if not bvp_match:
            raise ValueError(f"{split}: TEMP split metadata does not align with BVP metadata.")
        if not eda_match:
            raise ValueError(f"{split}: TEMP split metadata does not align with EDA metadata.")
...
def main() -> None:
    ...
    print("=" * 70)
    print("VITALAGENT - WESAD TEMP VERIFICATION")
    print("=" * 70)
    print(f"TEMP directory: {temp_dir}")
    print(f"Expected TEMP window: {WINDOW_SECONDS} seconds / {TEMP_WINDOW} samples at {TEMP_FS} Hz")

    X, y, metadata = load_dataset(temp_dir)
    validate_dataset("FULL TEMP", X, y, metadata)
    print_summary("FULL TEMP", dataset_summary(X, y, metadata))

    for split, subjects in SPLIT_SUBJECTS.items():
        X_split, y_split, metadata_split = load_dataset(temp_dir, split)
        name = f"{split.upper()} TEMP"
    ...
    verify_reference_alignment(temp_dir)

    print("\n" + "=" * 70)
    print("WESAD TEMP verification completed successfully.")
    print("=" * 70)
